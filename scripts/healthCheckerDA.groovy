...
ElementKey annotationTODO = null;

void start(OwlSchema schema) {
   annotationTODO = new ElementKey(schema, "SITAC", "Todo");
}
...
void process(NamedElement element) {
   ...
   if (element.getElementType().equals(ElementTypes.CLASS)) {
      ...
      if (hasTODO(theClass)) {
         logger.appendObjectLinks("Class %1 has a Todo", hyperLinkElt);
      }
      ...
      if (theProperty.isDatatypeProperty()) {
         if (hasTODO(theProperty)) {
            logger.appendObjectLinks("DataProperty %1 has a Todo", hyperLinkElt);
         }
...
boolean hasTODO(NamedElement element) {
   if (element instanceof AnnotatedElement) {
      AnnotatedElement annElt = (AnnotatedElement)element;
      return annElt.hasAnnotation(annotationTODO);
   } else {
     return false;
   }
}
